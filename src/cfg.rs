...
pub fn analyze_mil(program: &mil::Program) -> Graph {
    ...
    let direct = {
        ...
        add_virtual_entry_blocks(&mut direct, &mut bounds);
        direct
    };
...
fn add_virtual_entry_blocks(edges: &mut Edges, bounds: &mut Vec<mil::Index>) {
    ...
    let predecessors = compute_predecessors(&edges.successors);
    let end_ndx = *bounds.last().unwrap();
...
impl<T> BlockMultiMap<T> {
    ...
    fn add_block(&mut self) -> BlockID {
        // nothing else needs to change in self
        self.ndx_range.add_block(0..0)
    }
...
impl<T: Clone> BlockMap<T> {
    ...
    // private: we don't want to allow other modules to change the block count
    fn add_block(&mut self, value: T) -> BlockID {
        let len_pre = self.0.len();
        self.0.push(value);
        BlockID(len_pre.try_into().unwrap())
    }
